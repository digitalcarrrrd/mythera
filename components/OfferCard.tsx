  ...
  return (
    <div
      className={`relative flex flex-col justify-between p-8 sm:p-10 rounded-2xl transition-all duration-300 ${
        isRecommended
          ? 'bg-[#080a08] border-2 border-[#d8ff44] shadow-2xl shadow-[#d8ff44]/20 -translate-y-2 ring-1 ring-[#d8ff44]'
          : 'bg-[#080a08] border-2 border-[#ffffff15] hover:border-[#d8ff44]/50 shadow-lg'
      }`}
    >
      {/* Recommended Ribbon */}
      {isRecommended && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#d8ff44] text-[#000000] text-[10px] uppercase tracking-widest font-black px-4 py-1 rounded-full shadow-md">
          Recommended Choice
        </div>
      )}
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4 mb-3">
          <div>
            <span className="text-[10px] font-mono tracking-widest uppercase block text-[#d8ff44] font-bold">
              {offer.persona} OFFER
            </span>
        ...
        <p className="text-xs text-[#d8ff44] font-semibold tracking-wide mb-6">
          {offer.tagline}
        </p>
        ...
        {/* Meta Specifications */}
        <div className="grid grid-cols-2 gap-3 py-3 px-4 bg-[#0d100c] rounded-xl border border-[#ffffff10] text-[11px] text-[#9ea399] mb-6">
          <div className="flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-[#d8ff44]" />
            <span>{offer.turnaround}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <RefreshCw className="w-3.5 h-3.5 text-[#d8ff44]" />
            <span>{offer.revisions}</span>
          </div>
          <div className="flex items-center gap-1.5 col-span-2">
            <Mic className="w-3.5 h-3.5 text-[#d8ff44]" />
            <span>
        ...
        <div className="space-y-3 mb-8">
          ...
          {offer.features.map((feature, idx) => (
            <div key={idx} className="flex items-start gap-2.5 text-xs text-[#f3f3eb]">
              <div className="w-4 h-4 rounded-full bg-[#d8ff44]/20 flex items-center justify-center text-[#d8ff44] shrink-0 mt-0.5">
